...
def get_arm64_vivo():
    try:
        cursor = db["arm64_results"].find({"source": "live_engine"}, {"_id": 0}).sort("timestamp", -1).limit(1)
        ultimo = list(cursor)
        if not ultimo:
            return jsonify(None), 200
        return jsonify(ultimo[0]), 200
    except Exception as e:
        print(f"ERROR DETECTADO EN /api/historico/arm64: {e}")
        return jsonify({"error": str(e)}), 500
...
def analizar_datos_invernadero():
    try:
        ...
        config_rutinas = [
            ...
            {"id": "integral_error", "binario": "modulo_4_integral_error", "salida": "resultado_integral.txt"},
        ...
        documento_maestro= {
            ...
            "range": f"{linea_inicial}-{linea_final}",
            "column": columna,
        ...
        for rutina in config_rutinas:
            ...
            if not os.path.exists(ruta_binario):
                ...
                resultado_modulo["error_detalle"] = "Binario no encontrado."
            else:
                try:
                    ...
                    resultado_modulo["estado_ejecucion"] = "fallido"
                    resultado_modulo["error_detalle"] = "Timeout de ejecución."
                except Exception as e:
                    resultado_modulo["estado_ejecucion"] = "fallido"
                    resultado_modulo["error_detalle"] = str(e)

            documento_maestro["resultados"][rutina["id"]] = resultado_modulo

            estado_db = "OK" if resultado_modulo["estado_ejecucion"] == "exitoso" else "ERROR"
            
            doc_db = {
                "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "source": "historical_analyzer",
                "module": rutina["binario"],
                "range": f"{linea_inicial}-{linea_final}",
                "column": str(columna),
                "result": resultado_modulo["metricas"] if estado_db == "OK" else {},
                "status": estado_db,
                "error_detail": resultado_modulo["error_detalle"]
            }
            documentos_db.append(doc_db)

        if documentos_db:
            db["arm64_results"].insert_many(documentos_db)
