/*
    ...
    Salida:
       ...
       x2 = cantidad de números guardados (siempre debe ser 30)
...
.data
filename:
    .asciz "lecturas.csv"       // Nombre del archivo de entrada obligatorio
...
.bss
buffer:
    .skip 4096                  // Buffer amplio para leer todo el archivo CSV
write_buffer:
...
// ============================================================================
// 1. LECTURA Y EXTRACCIÓN DE COLUMNA AL STACK
// ============================================================================
utils_read_column_to_stack:
    ...
    mov x22, #0                 // x22 = contador de números extraídos (llegará a 30)

    bl utils_open_file          // Llamada para abrir lecturas.csv
    bl utils_read_file          // Llamada para volcar el contenido al buffer
    bl utils_close_file         // Llamada para cerrar el descriptor del archivo

    ldr x21, =buffer            // x21 = puntero para recorrer el buffer cargado en memoria

    bl utils_skip_to_next_line  // Saltar la primera línea (el encabezado del CSV)

    cmp w23, '$'                // Verificar si el archivo está vacío o solo tenía encabezado
    beq utils_done              // Si es el fin de archivo ('$'), terminar la ejecución
...
utils_find_column:
    ...
    beq utils_process_line      // Si es así, reiniciar búsqueda en la siguiente línea
...
utils_after_column:
    ...
    beq utils_process_line      // Si fue fin de línea, saltar directamente a procesar la siguiente fila
...
// ============================================================================
utils_write_result:
    stp x29, x30, [sp, #-16]!   // Guardar registros de retorno
    ...
    mov x2, #65                 // Banderas: O_WRONLY(1) | O_CREAT(64) = 65
    mov x3, #0777               // Permisos de creación del archivo (lectura/escritura/ejecución)
    ...
    ldp x29, x30, [sp], #16     // Restaurar estado del stack original
    ret                         // Retornar

// ============================================================================
// 3. OPERACIONES DE ARCHIVOS (OPEN, READ, CLOSE)
// ============================================================================
utils_open_file:
    mov x0, #-100               // AT_FDCWD (indica que busque en el directorio actual)
    ldr x1, =filename           // Cargar dirección del nombre del archivo "lecturas.csv"
...
utils_skip_to_next_line:
    ldrb w23, [x21], #1         // Cargar 1 byte a w23 y avanzar puntero (x21) 1 posición
    cmp w23, '$'                // Comparar con el carácter de fin de archivo
    beq utils_skip_done         // Terminar salto si encontramos EOF
    cmp w23, #10                // Comparar con salto de línea (\n en ASCII es 10)
    beq utils_skip_done         // Terminar salto si encontramos nueva línea
    b utils_skip_to_next_line   // Iterar recursivamente si es otro carácter

utils_skip_to_next_column:
    ldrb w23, [x21], #1         // Cargar 1 byte y avanzar puntero
    cmp w23, '$'                // Evaluar si es fin de archivo ($)
    beq utils_skip_done         // Detener si es EOF
    cmp w23, #10                // Evaluar si es nueva línea (\n)
    beq utils_skip_done         // Detener (columna vacía al final)
    cmp w23, ','                // Evaluar si es separador de CSV (coma)
    beq utils_skip_done         // Detener, encontramos la siguiente columna
    b utils_skip_to_next_column // Continuar consumiendo caracteres de esta columna
...
// --- utils_atoi ---
// Extrae los caracteres actuales hasta encontrar coma, espacio o enter
// y los convierte a un entero que queda almacenado en x10.
utils_atoi:
    mov x10, #0                 // Inicializar acumulador numérico en 0
    mov x5, #10                 // Base 10 para multiplicaciones decimales
atoi_loop:
    ldrb w23, [x21], #1         // Cargar siguiente carácter y avanzar puntero
