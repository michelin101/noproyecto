...
class Sensors:
    ...
    def _evaluar_estado_global(self):
        ...
        riesgos = [d.get("RISK", "LOW") for d in shared.arm64_decisions_list]
        acciones = [d.get("ACTION", "NO_ACTION") for d in shared.arm64_decisions_list]

        if "CRITICAL" in riesgos or "ALARM_ON" in acciones:
            shared.estado_global = "EMERGENCIA"
        elif "HIGH" in riesgos or "MEDIUM" in riesgos or shared.bomba_encendida:
