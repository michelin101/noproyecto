...
export default function Dashboard({ datos }) {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(fetchMotorArm64, 2000);
  ...
  return (
    <div className="grid-dashboard" style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
      ...
      <div className="card" style={{ flex: '1 1 300px' }}>
        ...
        <div style={{ fontSize: '12px', background: '#f8fafc', padding: '8px', borderRadius: '6px', overflowX: 'auto', color: 'black' }}>
          {motorArm64?.result && Array.isArray(motorArm64.result) ? (
            motorArm64.result.map((item, index) => (
              <div key={index} style={{ borderBottom: '1px solid #e2e8f0', paddingBottom: '4px', marginBottom: '4px' }}>
                <strong>{item.ACTION}</strong> | Riesgo: {item.RISK} | Razón: {item.REASON}
              </div>
            ))
          ) : (
            <pre>{motorArm64?.result ? JSON.stringify(motorArm64.result, null, 2) : 'N/A'}</pre>
          )}
        </div>
